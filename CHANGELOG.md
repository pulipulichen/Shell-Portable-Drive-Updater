...
- Fixed false-negative remote detection by replacing the `sed | rg` pipeline with a Bash-based `rclone listremotes` matcher.
- Added remote access probing before mount and automatic reconnect guidance when OAuth token errors (`invalid_grant` / `couldn't fetch token`) are detected.
- Hardened failure cleanup so unexpected interruptions still unmount and remove temporary mount directories safely.
- Fixed `rsync --exclude` prefix matching by escaping glob metacharacters, so prefixes like `[` now correctly exclude matching files/directories.
